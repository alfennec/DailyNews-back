...
?>
	...
	<?php
	    ...
	    $sql_query = "SELECT image FROM notification WHERE id = ?";
		...
		if (isset($_POST['btnEdit'])) {
            ...
            if (!empty($menu_image)) {
                ...
                $sql_query = "UPDATE notification SET title = ?, message = ?, image = ?, url_notification = ?, modified = ? WHERE id = ?";

                $upload_image = $category_image;
                $stmt = $connect->stmt_init();
                if ($stmt->prepare($sql_query)) {
                    // Bind your variables to replace the ?s
                    $stmt->bind_param('ssssss', $title, $message, $upload_image, $link, $modified, $ID);
                    // Execute query
                    $stmt->execute();
                    // store result
				...
				$sql_query = "UPDATE notification SET title = ?, message = ?, url_notification = ?, modified = ? WHERE id = ?";
					
				$stmt = $connect->stmt_init();
				if($stmt->prepare($sql_query)) {	
					// Bind your variables to replace the ?s
					$stmt->bind_param('sssss', $title, $message, $link, $modified, $ID);
					// Execute query
					$stmt->execute();
					// store result 
					$update_result = $stmt->store_result();
...
EOF;
		...
		$sql_query = "SELECT id, title, message, image, url_notification, modified, created FROM notification WHERE id = ?";
                                ...
                                <div>
                                    ...
                                    <div class="form-group form-float col-sm-12">
                                        <div class="form-line">
                                            ...
                                            <input type="text" class="form-control" name="link" id="link" placeholder="http://www.your-url.com" value="<?php echo $data['url_notification']; ?>" >
                                        </div>
                                    </div>                           
