...
<?php
    ...
    if (isset($_POST['btnAdd'])) 
    {
		...
		if (!empty($title) && !empty($message) && empty($error['image'])) {
			...
			$sql_query = "INSERT INTO notification (title, message, image, url_notification, created, modified) VALUES (?, ?, ?, ?, ?, ?)";
					
			$stmt = $connect->stmt_init();
			if ($stmt->prepare($sql_query)) {	
				// Bind your variables to replace the ?s
				$stmt->bind_param('ssssss', $title, $message, $upload_image, $link, $created, $modified);
				// Execute query
				$stmt->execute();
				// store result 
