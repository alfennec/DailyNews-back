<?php
    ...
    if (isset($_GET['id']))
    {
        ...
        $sql_query = "DELETE FROM notification WHERE id = ?";
...
?>
    ...
    <?php 
        ...
        if (empty($keyword)) {
            $sql_query = "SELECT id, title, message, image, url_notification, created, modified FROM notification ORDER BY id DESC";
        } else {
            $sql_query = "SELECT id, title, message, image, url_notification, created, modified FROM notification WHERE title LIKE ? ORDER BY id DESC";
        }
        ...
        if (empty($keyword)) {
            $sql_query = "SELECT id, title, message, image, url_notification, created, modified FROM notification ORDER BY id DESC LIMIT ?, ?";
        } else {
            $sql_query = "SELECT id, title, message, image, url_notification, created, modified FROM notification WHERE title LIKE ? ORDER BY id DESC LIMIT ?, ?";
        }
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <h2>MANAGE NOTIFICATION</h2>
                            <div class="header-dropdown m-r--5">
                                <a href="add-notification.php"><button type="button" class="btn bg-blue waves-effect">Add New Notification</button></a>
                            </div>
                            <br>
                                ...
                                <?php 
                                    while ($stmt_paging->fetch()) { ?>
                                        <tr>
                                            ...
                                            <td><?php echo $data['url_notification'];?></td>
                                            
                                            <td>
                                                <a href="send-notification.php?id=<?php echo $data['id'];?>">
                                                ...
                                                <a href="push-notification.php?id=<?php echo $data['id'];?>" onclick="return confirm('Are you sure want to delete this Notification?')">
